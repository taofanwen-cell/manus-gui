...
| D-19 | **"正常场景不会触发"的标注/分支必须有断言守着** | 快照日期标注在 shop_total 场景根本不渲染，坏了一眼看不出；scripts/ 已两次栽在"没有测试守着"（B-78 子项 / B-77） |
| D-20 | **提交/推送前跑 `bash scripts/preflight.sh`** | 10 秒回答"改坏没 / 落盘没"：ruff F821 + import 冒烟 + 测试全套，专治"编辑假成功"与"以为改了没改" |
...
**上次更新**：2026-09-12（补快照日期标注的断言缺口（D-19）：同 goods_id 时报告须出现「单品销量 (快照 YYYY-mm-dd)」+「数据时效提示」、异 goods_id 时整行锚定列表页 top1 不得含「快照」——此前该标注全测试文件只有一行注释、在 OPPO 这类 shop_total 场景根本不触发，坏了看不出；修单品牌退化结论「X 切入最低价位…X 价位最高」（D-20 前置）；新增 `scripts/preflight.sh` 交付前自检（ruff F821 + import 冒烟 + 测试全套）并写进工程纪律 D-19/D-20；删除过期的 `data/ecommerce_brand_compare_VERIFY.md`（手工 replay 产物，已被回归测试 + preflight 取代））
